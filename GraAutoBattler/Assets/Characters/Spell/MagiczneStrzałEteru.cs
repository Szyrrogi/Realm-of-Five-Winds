...
public class MagiczneStrzałEteru : Spell
{
    public override IEnumerator Fight()
    {
        ...
        if(unit.findPole() != null && unit.findPole().GetComponent<Pole>().unit != null && unit.Enemy != unit.findPole().GetComponent<Pole>().unit.GetComponent<Unit>().Enemy)
        {
            ...
            yield return StartCoroutine(enemyUnit.TakeDamage(this, enemyUnit.BeforDamage(gameObject, BeforAttack(enemyUnit.gameObject, AP * 2)),TypeDamage.typeDamage.Magic));
        }
        else
        {
