...
public class SynergyTruaDamage : Synergy
{
    ...
    public IEnumerator Buff(int linenumber)
    {
        ...
        foreach (var pole in line.pola)
        {
            if (pole.unit != null && pole.unit.GetComponent<Unit>().Name == units[0].Name)
            {
                ...
                if(pole.unit.GetComponent<Włucznik>())
                {
                    pole.unit.GetComponent<Włucznik>().Synergy = true;
                }  
                if(pole.unit.GetComponent<TancerzOstrzy>())
                {
                    pole.unit.GetComponent<TancerzOstrzy>().Synergy = true;
                }
            }
        }
    }
