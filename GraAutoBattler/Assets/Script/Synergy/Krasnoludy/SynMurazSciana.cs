...
public class SynMurazSciana : Synergy
{
    ...
    public IEnumerator Buff(int linenumber)
    {
        ...
        foreach (var pole in line.pola)
        {
            if (pole.unit != null && pole.unit.GetComponent<Unit>().Name == units[0].Name)
            {
                ...
                pole.unit.GetComponent<Unit>().ShowPopUp("+" + pole.unit.GetComponent<Unit>().Health , Color.yellow);
                pole.unit.GetComponent<Unit>().Attack = pole.unit.GetComponent<Unit>().Health;      
                pole.unit.GetComponent<Sciana>().Synergia = true;   
            }
        }
    }
